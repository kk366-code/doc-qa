...
- **重い処理の前のチェックポイント・コミット**:
- `uv add` や `git clone` などで、**数分以上かかると予想される大きなバイナリ（ONNXモデル等）や大量の依存関係をダウンロードする前**、あるいは**ダウンロードが始まって時間がかかっていると判断した際**には、その直前の状態（`pyproject.toml` の変更など）を一旦 `WIP: checkpoint before large download` などのメッセージでコミットし、作業内容を保護すること。
