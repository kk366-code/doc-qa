...
- 以下の API を有効化: Cloud SQL, Cloud Run, Artifact Registry, Secret Manager, Cloud Build
...
```bash
# DATABASE_URL: Cloud SQL Proxy (Unix ソケット) 経由の接続文字列
...
Cloud Run はサービスアカウント経由で Vertex AI に直接認証できる（ADC ファイル不要）。

```bash
gcloud projects add-iam-policy-binding $PROJECT_ID \
  --member="serviceAccount:$SA" --role=roles/aiplatform.user

gcloud run services update rag-demo --region=$REGION \
